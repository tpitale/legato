  ...
  module Management
    class Filter
      ...
      def self.default_path
        "/accounts/~all/webproperties/~all/profiles/~all/filters"
      end
      ...
      def self.for_account(account)
        all(account.user, account.path+'/webproperties/~all/profiles/~all/filters')
      end

      def self.for_web_property(web_property)
        all(web_property.user, web_property.path+'/profiles/~all/filters')
      end

      def self.for_profile(profile)
        all(profile.user, profile.path+'/filters')
      end
    end
  end
end
